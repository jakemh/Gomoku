...
if VERSION == 1
  ...
  def self.gameOver(board, player, move = nil)
    ...
    { temp_data: { status: player == 0 ? 'black_winner' : 'white_winner', chain: json_chain, win_coord: move,
    ...
    movePair = @m.negaMaxWithTimer(board, 1, 1, -999999, 999999)
    ...
    if @m.didTimeExpire == true
      ...
      # else
      #   puts "***BMT DEAD***"
         multiAgent = MultiAgentSearch.new(board_copy, { 'maxDepth' => java.lang.Integer.new(4),
                                                        'movesConsidered' => java.lang.Integer.new(this_game.moves_considered),
                                                        'aggressiveness' => java.lang.Integer.new(this_game.aggressiveness),
                                                        'defensiveness' => java.lang.Integer.new(this_game.defensiveness)
         ...
         # backupPair = m.minMaxAB(board, 1, 1, -999999, 999999)
         backupPair = multiAgent.negaMaxWithTimer(board_copy, 1, 1, -99999, 999999)
    ...
    # this_game.update_attributes({:board => board0.to_json, :temp_data => move_array.to_json})
      ...
      if board2.boardFull
        ...
        print 'WHITE WINS*****'
              return gameOver(board2, 1, move_array)


      end
      board.print

    { board: board0.to_json,
