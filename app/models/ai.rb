...
module AI
  ...
  def self.gameOver(board, player, p2_moves)
    ...
    return {:p2_moves => p2_moves, :status => player == 0 ? "black_winner" : "white_winner", :win_chain_array => chain_array}
   end
  ...
  def self.get_board_from_game_obj(this_game)
    ...
    board = self.array_grid_to_board_obj(board0, this_game.win_chain)
    return board
  end

  def self.get_move_pair(multiAgent, board)
    return multiAgent.negaMaxWithTimer(board, 1, 1, -999999, 999999)
  end
  ...
  def self.move_pair(board, this_game) #get data from java code
    ...
    @mainMultiAgent = MultiAgentSearch.new(board, self.optionsHash(this_game.depth, this_game))
    backupMultiAgent = MultiAgentSearch.new(board_copy, self.optionsHash(4, this_game))

    movePair = self.get_move_pair(@mainMultiAgent, board)

    print "*$*move pair*$* ", movePair
    if @mainMultiAgent.didTimeExpire == true
      backupPair = self.get_move_pair(backupMultiAgent, board_copy)
  ...
  def self.game_data(this_game, win_chain)
    ...
    return self.gameOver(board, 0,  p2_moves.push(nil)) if board.isLose  
    # return self.gameOver(board, 1) if board.isWin
    # board = Board.buildFromInput("gomokuproj2/input/", "3")
    movePair = self.move_pair(board, this_game) #getting key data
    ...
    # return self.gameOver(board2, 0, p2_moves.push(move_array)) if board2.isLose  
    return self.gameOver(board2, 1, p2_moves.push(move_array)) if board2.isWin
